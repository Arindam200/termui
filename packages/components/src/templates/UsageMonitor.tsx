...
function statusDotChar(status: 'green' | 'yellow' | 'red'): { char: string; color: string } {
  switch (status) {
    case 'green':
...
function UsageMonitorRoot({
  ...
  // Force re-render on interval for live data
    ...
    <Box flexDirection="column">
      {React.Children.map(children, (child, i) => (
        <React.Fragment key={i}>
          {child}
          {/* Auto separator between sections except after StatusBar */}
          {i < React.Children.count(children) - 1 &&
            React.isValidElement(child) &&
            (child.type as any).displayName !== 'UsageMonitor.StatusBar' && (
...
function UsageMonitorTags({ items, bracketColor, separatorColor }: UsageMonitorTagsProps) {
  return (
    <Box flexDirection="row" marginBottom={1}>
      ...
      {items.map((item, i) => (
        <React.Fragment key={i}>
          <Text>{item}</Text>
          {i < items.length - 1 && <Text color={separatorColor ?? 'cyan'}>{' | '}</Text>}
        </React.Fragment>
      ))}
      <Text color={bracketColor ?? 'cyan'}>{' ]'}</Text>
    </Box>
...
function UsageMonitorSection({ icon, title, subtitle, children }: UsageMonitorSectionProps) {
  return (
...
function UsageMonitorDistributionMetric({
  icon,
  label,
  segments,
  barWidth = 22,
}: UsageMonitorDistributionMetricProps) {
  // Build a proportional bar from segments
...
function UsageMonitorStats({ children }: { children: ReactNode }) {
  return <Box flexDirection="column">{children}</Box>;
}

function UsageMonitorStatRow({
  icon,
  label,
  value,
  valueSuffix,
...
function UsageMonitorPredictions({ children }: { children: ReactNode }) {
  return (
    <Box flexDirection="column">
...
function UsageMonitorPrediction({
  label,
  value,
  valueColor = 'yellow',
...
function UsageMonitorStatusBar({
  clock = false,
  clockColor,
  sessionLabel,
  sessionColor = 'green',
  ...
  return (
    <Box flexDirection="row" paddingTop={1}>
      ...
      {sessionLabel && <Text color={sessionColor}>{separator + '📄 ' + sessionLabel}</Text>}
