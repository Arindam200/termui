...
Use `onProcessExit()` from `@termui/adapters/args` for a cross-platform exit handler that works on all three platforms:

```ts
import { onProcessExit } from '@termui/adapters/args';
...
```ts
import { downsampleColor, nearestAnsi256, nearestAnsi16 } from '@termui/core';
